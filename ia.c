...
/*
COORDENADAS DA IA, x,y= posi��o do primeiro disparo em um navio (usado para atirar do outro lado caso n�o destrua)                    z= AUXILIA A IA EM ATIRAR NOS REDORES CASO ACERTE UM TIRO, SE ACERTAR UM TIRO z VIRA VERDADEIRO
*/
int x = 0, y = 0, z = 0, I, J;
...
char
colunaInimigo (int j)
{                      // CONVERTE A COLUNA DA IA PARA O CARACTERE CORRESPONDENTE(USADO APENAS PARA O PRINTF)
    char x;
    switch (j)
    {
        case 1:
...
void
TiroCertoIA (void)
{                             // PISCA A TELA EM VERMELHO E FAZ SOM
    //Beep(400,50);
    system("color 40");
    system("color C0");
    system("color 40");
...
void
ZeraBaseIA (void)
{                              // ZERA A MATRIZ DE BASE(INT) DA IA
    int i, j;
    for (i = 0; i < LIN; i++)
    {                 // ZERANDO AS BASES
        for (j = 0; j < COL; j++)
...
void
BordaBaseIA (void)
{                             // DA VALOR "EDGE" �S BORDAS DA MATRIZ DE BASE(INT)
    int i, j;
    for (i = 0; i < LIN; i++)
    {                 // NUMERANDO AS BORDAS
        for (j = 0; j < COL; j++)
...
void
GeraNaviosIA (int base[][COL])
{             // GERA OS NAVIOS DA IA ALEARTORIAMENTE
    int v, c, i, j; // v=Navio na vertical ou horizontal, c= Limita quantas vezes vai repetir os la�os, i,j= coordenadas
    for (c = 0; c < 4; c++)
...
void
GeraTiroIA (int i, int j)
{                              // GERA AS COORDENADAS DO TIRO DA IA
    do{
        i=(rand()%(LIN-2))+1;
        j=(rand()%(COL-2))+1;
    }while(tabIA[i][j]!='~'||tabIA[i+1][j]=='x'||tabIA[i-1][j]=='x'||
    tabIA[i][j+1]=='x'||tabIA[i+1][j+1]=='x'||tabIA[i-1][j+1]=='x'||
    tabIA[i][j-1]=='x'||tabIA[i+1][j-1]=='x'||tabIA[i-1][j-1]=='x');  // VERIFICA SE O TIRO JA FOI DADO NO LOCAL E SE ALGUM NAVIO EST� NOS ARREDORES
    I=i;
    J=j;
}
...
void
ProximoTiroIA (void)
{                           // FUN��O USADA PARA ATIRAR NOS ARREDORES
    if (tabIA[I+1][y]=='~'&&tabIA[I][y]=='*'&&tabIA[I+2][y]!='x')
    {   // OLHA SE O TEM AGUA ABAIXO E SE ACERTOU O TIRO ANTERIOR
        ++I;
    }
    else
    {                                                           // SE N�O
        if (tabIA[I][y]==' '||I==LIN-1||tabIA[I+1][y]==' ')
        {             // OLHA SE ERROU O TIRO ANTERIOR
            I=x;                                                            //SE ERROU, IGUALA i AO VALOR DO PRIMEIRO TIRO ACERTADO
        }
        if (tabIA[I-1][y]=='~'&&tabIA[I][y]=='*'&&tabIA[I-2][y]!='x')
        {   // OLHA SE ACIMA TEM AGUA E SE ACERTOU O TIRO ANTERIOR
            --I;
        }
        else{                                                           // SE N�O
            if(tabIA[x][J+1]=='~'&&tabIA[x][J]=='*'&&tabIA[x][J+2]!='x')
            {   // OLHA SE A DIREITA TEM AGUA E SE ACERTOU O TIRO ANTERIOR
                ++J;
            }
            else{                                                           // SE N�O
                if(tabIA[x][J]==' '||J==COL-1||tabIA[x][J+1]==' ')
                {             // OLHA SE ERROU O TIRO ANTERIOR
                    J=y;
                }
                if(tabIA[x][J-1]=='~'&&tabIA[x][J]=='*'&&tabIA[x][J-2]!='x')
                {   // OLHA SE A ESQUERDA TEM AGUA E SE ACERTOU O TIRO ANTERIOR
                    --J;
                }
            }
        }
...
int
VerificaAcertoIA (int i, int j)
{              // VERIFICA SE A IA ACERTOU O TIRO
    if(baseIA[i][j] == N1_IA)
    ...
    {                     // SE ACERTAR O NAVIO DE 2
        ...
        printf("IA: ");
        printf("Acertou o tiro!\n");
        if(tabIA[i+1][j]=='*'||tabIA[i][j+1]=='*'||tabIA[i-1][j]=='*'||tabIA[i][j-1]=='*')
        {  // SE O NAVIO FOI DESTRUIDO TRANSFORMA * EM x
            tabIA[i][j]='x';
            if(tabIA[i+1][j]=='*')
            {
                tabIA[i+1][j]='x';
